    ...
    <div className="space-y-3 mb-4">
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <div className="relative flex-1 min-w-[180px] max-w-xs">
          ...
          <input
            className="input pl-8 py-1.5 text-sm bg-dark-bg"
        ...
        <div ref={filterRef} className="flex items-center gap-1">
          ...
          <div className="relative">
            ...
            {showFilter && (
              <div className="absolute top-full left-0 mt-1 w-56 rounded-md border border-dark-border bg-dark-card p-3 z-30 shadow-lg">
          ...
          {/* Sort */}
          <div className="relative">
            <button onClick={() => { setShowSort(!showSort); setShowFilter(false); setShowGroup(false); setShowColumns(false); }} className="toolbar-btn">
              <ArrowUpDown className="h-3.5 w-3.5" />
              Sort
            </button>
            {showSort && (
              <div className="absolute top-full left-0 mt-1 w-48 rounded-md border border-dark-border bg-dark-card p-2 z-30 shadow-lg">
                {[
          ...
          {/* Group */}
          <div className="relative">
            <button onClick={() => { setShowGroup(!showGroup); setShowFilter(false); setShowSort(false); setShowColumns(false); }} className="toolbar-btn">
              <Layers className="h-3.5 w-3.5" />
              Group
              {groupBy !== 'none' && <span className="text-2xs">· {groupBy}</span>}
            </button>
            {showGroup && (
              <div className="absolute top-full left-0 mt-1 w-40 rounded-md border border-dark-border bg-dark-card p-2 z-30 shadow-lg">
          ...
          {visibleColumns && onToggleColumn && (
            <div className="relative">
              <button onClick={() => { setShowColumns(!showColumns); setShowFilter(false); setShowSort(false); setShowGroup(false); }} className="toolbar-btn">
                <Columns3 className="h-3.5 w-3.5" />
                Columns
              </button>
              {showColumns && (
                <div className="absolute top-full left-0 mt-1 w-44 rounded-md border border-dark-border bg-dark-card p-2 z-30 shadow-lg">
          ...
          {/* View selector */}
          {showViewSelector && (
            <div className="flex items-center rounded-md border border-dark-border p-0.5 ml-1">
              <button
        ...
        {totalCount !== undefined && (
          <span className="text-2xs text-text-muted hidden sm:inline">{totalCount} tasks</span>
        )}

        <Button onClick={onNewTask} size="sm" className="ml-auto gap-1.5 shrink-0">
